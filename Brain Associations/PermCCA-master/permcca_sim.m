function varargout = permcca_sim(varargin)
...
% disp(['Y-A-U size: ', num2str(size(Y)), '-', num2str(size(A)), '-', num2str(size(U))]);
% disp(['X-B-V size: ', num2str(size(X)), '-', num2str(size(B)), '-', num2str(size(V))]);
% disp(['K size: ', num2str(size(K))]);

% Initialise counter
cnt = zeros(1,K);
lW  = zeros(1,K);
Aperms = zeros(size(A,1), size(A,2), nP);
...
for p = 1:nP
    ...
    % permute without loop over caninocal variables
    [AA,~,~] = cca(Qz*U(idxY,:),Qw*V(idxX,:),R,S);
    Aperms(:,:,p) = AA;
    ...
    for k = 1:K
        ...
        % disp(['AA size:', num2str(size(AA))]);
        % disp(['rperm size:', num2str(size(rperm))]);
        lWtmp = -fliplr(cumsum(fliplr(log(1-rperm.^2))));
        lW(k) = lWtmp(1);
        %%%%%%%%%
        % save AA for Y/brainvars per iteration
        % Aperms(:,:,k) = AA;
        %%%%%%%%%
    end
    if p == 1
